...
program
  ...
  .option('-p, --server-port <port>', 'Server port', (val) => parseInt(val, 10))
  .option('-l, --local-port <port>', 'Local port', (val) => parseInt(val, 10) || 1081)
  .option('-k, --password <password>', 'Password')
  .option('-m, --method <method>', 'Encryption method', 'aes-256-gcm')
  .option('-t, --timeout <seconds>', 'Connection timeout in seconds', (val) => parseInt(val, 10) || 60)
